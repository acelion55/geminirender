        ...
        while attempt < max_attempts:
            ...
            () => {
                ...
                for (let img of imgs) {
                    ...
                    // Ignore small avatars, icons, logos
                    if (src.includes('avatar') || src.includes('profile') || src.includes('auth0') || src.includes('googleusercontent') || src.endsWith('.svg')) continue;

                    if (src.includes('oaidalleapiprod') || 
                        src.includes('files.oaiusercontent.com') || 
                        src.includes('oaiusercontent') ||
                        alt.toLowerCase().includes('generated by dall') ||
                        alt.toLowerCase().includes('dall-e') ||
                        img.closest("div[data-message-author-role='assistant']")) {
            ...
            if result and result.get("src"):
                ...
                b64_js = """
                async (targetSrc) => {
                    // Method 1: Draw onto Canvas (fast, no CORS error if loaded in DOM)
                    try {
                        const imgs = Array.from(document.querySelectorAll('img'));
                        const targetImg = imgs.find(i => i.src === targetSrc);
                        if (targetImg && targetImg.complete && targetImg.naturalWidth > 50) {
